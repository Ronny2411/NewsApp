...
fun OnBoardingPage(
    ...
    Column(modifier = modifier) {
        ...
        Text(text = page.title, modifier = Modifier.padding(horizontal = Dimens.MediumPadding2),
            style = MaterialTheme.typography.displaySmall.copy(fontWeight = FontWeight.Bold),
            color = colorResource(id = R.color.display_small))
        Text(text = page.desc, modifier = Modifier.padding(horizontal = Dimens.MediumPadding2),
...
fun OnBoardingPagePrev(){
    NewsAppTheme {
        OnBoardingPage(page = pages[0],Modifier)
    }
}